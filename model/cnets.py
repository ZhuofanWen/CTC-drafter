...
def _make_causal_mask(
    ...
    mask = torch.full((tgt_len, tgt_len), torch.finfo(dtype).min, device=device)#获取dtype范围最小数
    mask_cond = torch.arange(mask.size(-1), device=device)
    mask.masked_fill_(mask_cond < (mask_cond + 1).view(mask.size(-1), 1), 0)#对角线及下方的值为1
    mask = mask.to(dtype)

    if past_key_values_length > 0:
        mask = torch.cat([torch.zeros(tgt_len, past_key_values_length, dtype=dtype, device=device), mask], dim=-1)#在mask 每一行将past_key_values位置置1，所有都可以atten之前的value
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length)#causal_mask:[bsz,1,tgt_len,tgt_len+past_key_values_len]
...
class Model(nn.Module):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        # create causal mask
        # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len(tgt_len+past_val_len)]
        ...
        if attention_mask is not None:
            ...
            )#应该是causal_mask是固定的mask，用户自定义mask经expand后与causal_mask相加

        # [MODIFIED] add tree mask
        if hasattr(self, "tree_mask") and self.tree_mask is not None:
            tree_mask = self.tree_mask
            tree_len = tree_mask.size(-1)#新添加的tree那一部分的mask
            combined_attention_mask[:, :, -tree_len:, -tree_len:][
                tree_mask == 0
                ] = torch.finfo(torch.float32).min#原本0的位置变为最小值
    ...
    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids, head, use_cache=True):
        ...
        if use_cache:
            ...
            for i in range(3):
                ...
                select_top = torch.tensor([0],device=input_ids.device)
                select_index=topk_index[select_top]
                input_ids=select_index[None,:]
                if i==0:
                    hidden_states = out_hidden[:, -1:]
                else:
                    hidden_states=out_hidden
                self.tree_mask=torch.tensor([[[[1]]]],device=input_ids.device)
                position_ids = torch.tensor([0],device=input_ids.device)
                position_ids=len_posi+position_ids
